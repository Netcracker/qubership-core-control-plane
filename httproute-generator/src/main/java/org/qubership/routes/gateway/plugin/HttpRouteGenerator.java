...
public class HttpRouteGenerator {
    ...
    private static List<HTTPRouteResource> createHttpRoutes(
            String serviceName,
            int servicePort,
            Set<HttpRoute> httpRoutes
    ) {
        Map<HttpRoute.Type, List<HttpRoute>> routesByType =
                httpRoutes.stream().collect(Collectors.groupingBy(HttpRoute::type));
        ...
        return routesByType.entrySet().stream()
                .map(entry -> {
                    ...
                    List<HTTPRouteResource.Spec.BackendRef> backendRefs = List.of(
                            new HTTPRouteResource.Spec.BackendRef(
                                    serviceName,
                                    servicePort
                            )
                    );
                    ...
                    HTTPRouteResource.Spec spec = new HTTPRouteResource.Spec(
                            Set.of(parentRef),
                            ruleList
                    );
