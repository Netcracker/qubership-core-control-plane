...
This relates only to the egress gateway deployment, but routes for egress should be migrated by the route configuration owners. Their mapping, TLS origination and the behavior differences between the two meshes are covered in [Egress Routing](../egress-routing.md).
