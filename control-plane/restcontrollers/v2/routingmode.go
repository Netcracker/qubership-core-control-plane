	...
	return func(ctx fiber.Ctx) error {
		...
		for _, request := range routeRegistrationReq {
			if err := c.validateSingleRouteRegistrationRequest(ctx, &request); err != nil {
				logger.Debugf("ValidateRoutesApplicabilityToCurrentRoutingMode request validation error: %v", err)
				return restutils.RespondWithError(ctx, http.StatusBadRequest, fmt.Sprintf("ValidateRoutesApplicabilityToCurrentRoutingMode request validation error: %s", err))
			}
		}
		return ctx.Next()
	}
...
func (c *RoutingModeController) validateSingleRouteRegistrationRequest(ctx fiber.Ctx, routeRegistrationReq *dto.RouteRegistrationRequest) error {
	...
	if c.service.IsForbiddenRoutingMode(version, ns) {
		...
		logger.Warnf("Routes for cluster %s are not registered", routeRegistrationReq.Cluster)
		errMsg := fmt.Sprintf(
