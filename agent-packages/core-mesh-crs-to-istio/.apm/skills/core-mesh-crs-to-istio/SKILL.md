...
| `FacadeService` | omitted — equal to a mesh-type Gateway; may reference a `Gateway` via `spec.gateway` |
...
- [ ] No `FacadeService` appears in autogenerated files; no `mesh` type Gateways — only their derived HTTPRoutes
